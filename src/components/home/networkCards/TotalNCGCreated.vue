  ...
  <button class="network-card">
    <div class="network-card-wrapper">
      <div class="icon-field">
        <div class="icon-base">
          <img src="@/assets/images/icons/magic-wand.svg" alt="" />
        </div>
      </div>
      <div class="content-field">
        <p class="data-text">{{ Number(totalNCG).toLocaleString() }}</p>
...
export default {
  name: "TotalNCGCreated",
  setup() {
    const showIcon = "";
    const titleText = "Total NCG Created";
    ...
    const blockList = useResult(
      result,
      null,
      (data) => data.chainQuery.blockQuery.blocks[0].index
    );

    console.log(result);

    const totalNCG = computed(() => {
      return blockList.value * 10;
    });

    return { showIcon, titleText, blockList, totalNCG, loading, error };
  },
};
</script>
